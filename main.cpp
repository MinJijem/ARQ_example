    ...
    while(1)
    {
        ...
        switch (main_state)
        {
            case MAINSTATE_IDLE: //IDLE state description
                ...
                if (arqEvent_checkEventFlag(arqEvent_dataRcvd)) //if data reception event happens event c
                {
                    ...
                    arqMsg_encodeAck(arqAck, arqMsg_getSeq(dataPtr)); // ACK gen
                    arqLLI_sendData(arqAck, ARQMSG_ACKSIZE, srcId); // send ACK
                ...
                else if (arqEvent_checkEventFlag(arqEvent_dataToSend)) //if data needs to be sent (keyboard input) event a
                {
                    //msg header setting
